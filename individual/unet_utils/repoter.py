...
class Reporter:
    ...
    def save_image(self, train, test, epoch, m):
        file_name = self.IMAGE_PREFIX + str(epoch) + str(m)+self.IMAGE_EXTENSION
    ...
    def save_image_from_ndarray(self, train_set, test_set, palette, epoch, m, index_void=None):
        ...
        self.save_image(train_image, test_image, epoch, m)
    ...
    def save_loss_model(self, model_unet, gpu):
        #saver.save(sess, os.path.join(self._model_dir, self.MODEL_NAME))
        torch.save(model_unet.state_dict(), os.path.join(self._result_dir,'loss_model_{0}'.format(gpu)))
        
    def save_accu_model(self, model_unet, gpu):
        #saver.save(sess, os.path.join(self._model_dir, self.MODEL_NAME))
        torch.save(model_unet.state_dict(), os.path.join(self._result_dir,'accu_model_{0}'.format(gpu)))
