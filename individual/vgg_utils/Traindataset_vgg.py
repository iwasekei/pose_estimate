...
class Train(object):
    ...
    def __init__(self, raw_data, image_path, seg_path, imsize, rotate_range, scale_range, sym_combi, c_deviation=2):
        for key, val in locals().items():
            setattr(self, key, val)
        self.load_data(self.raw_data,self.image_path, seg_path)
    ...
    def load_data(self, raw_data,image_path, seg_path):
        ...
        with open(raw_data,"r") as f:
            ...
            for row in tqdm(reader):
                ...
                joint_x = list(map(float, row[5::2]))#
                joint_y = list(map(float, row[6::2]))#
                self.im_path += [path]
                ...
                if not (path in self.images):
                        self.images[path] = cv.imread("{0}/{1}.jpg".format(image_path,path))

                
        print ('ready!')

    def generate(self, i, label):
        path = self.im_path[i]
