...
if __name__ == '__main__' :
    ...
    for epoch in range(args.n_epoch):
        ...
        for i in tqdm(range(0, len(data), args.batchsize)):
            ...
            r_label, vgg_loss = model.vgg16(imgs,labels)
            ...
            # Training
            unet_h, unet_loss, dam_unet = model.unet(inputs_torch, teacher_torch, dam)
            ...
            #labels = torch.from_numpy(np.array(labels).astype(np.float32))#
            #labels = labels.to(device)
            h, cpm_loss = model.cpm(imgs, c_maps, b_maps, r_label, dam_unet)
        ...
        for i in tqdm(range(len(eva))):
            ...
            r_label, loss = model.vgg16(img, label)#
            ...
            h2, loss, dam_unet = model.unet(inputs_torch, teacher_torch, dam)
            ...
            #label = torch.from_numpy(np.array(label).astype(np.float32))#
            #label = label.to(device)
            h, loss = model.cpm(img, c_map, b_map,r_label, dam_unet)#
        ...
        if minloss>(testloss/len(eva)):
            ...
            torch.save(model.state_dict(), os.path.join(args.dir,'trained_model','{0}'.format(args.gpu)))
