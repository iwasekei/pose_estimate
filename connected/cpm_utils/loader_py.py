...
class Loader(object):
    def __init__(self, image_path, seg_path, init_size=(128, 128), one_hot=True):
        self._data = Loader.import_data(image_path, seg_path, init_size, one_hot)

    def get_all_dataset(self):
        return self._data

    def load_train_test(self, train_rate=0.875, pattern=0, shuffle=False, transpose_by_color=False):
        """
        ...
        train_test_set = self._data.perm_train(pattern, shuffle=False)#
        train_set = self._data.perm_train(pattern, shuffle=True)
        val_set = self._data.perm_val(pattern)
        test_set = self._data.perm_test(pattern)

        return train_set, train_test_set, val_set, test_set

    @staticmethod
    def import_data(image_path, seg_path, init_size=None, one_hot=True):
        # Generate paths of images to load
        # 読み込むファイルのパスリストを作成
        #paths_original, paths_segmented = Loader.generate_paths(dir_original, dir_segmented)
        #paths_original.sort()
        #paths_segmented.sort()

        # Extract images to ndarray using paths
        # 画像データをndarrayに展開
        images_original, images_segmented = Loader.extract_images(image_path, seg_path, init_size, one_hot)

        # Get a color palette
        # カラーパレットを取得
        image_sample_palette = Image.open(seg_path[0])
    ...
    def extract_images(paths_original, paths_segmented, init_size, one_hot):
        ...
        images_segmented = np.where(images_segmented != 0, 1, images_segmented)
...
class DataSet(object):
    ...
    def perm_train(self, pattern, shuffle):
        if pattern ==0:
            img = self._images_original[0:840]
            seg = self._images_segmented[0:840]
            
        elif pattern ==1:
            img1 = self._images_original[0:700]
            img2 = self._images_original[980:1120]
            seg1 = self._images_segmented[0:700]
            seg2 = self._images_segmented[980:1120]
            img = np.concatenate([img1,img2])
            seg = np.concatenate([seg1,seg2])
            
        elif pattern==2:
            img1 = self._images_original[0:560]
            img2 = self._images_original[840:1120]
            seg1 = self._images_segmented[0:560]
            seg2 = self._images_segmented[840:1120]
            img = np.concatenate([img1,img2])
            seg = np.concatenate([seg1,seg2])
            
        elif pattern==3:
            img1 = self._images_original[0:420]
            img2 = self._images_original[700:1120]
            seg1 = self._images_segmented[0:420]
            seg2 = self._images_segmented[700:1120]
            img = np.concatenate([img1,img2])
            seg = np.concatenate([seg1,seg2])
        
        elif pattern==4:
            img1 = self._images_original[0:280]
            img2 = self._images_original[560:1120]
            seg1 = self._images_segmented[0:280]
            seg2 = self._images_segmented[560:1120]
            img = np.concatenate([img1,img2])
            seg = np.concatenate([seg1,seg2])
        
        elif pattern==5:
            img1 = self._images_original[0:140]
            img2 = self._images_original[420:1120]
            seg1 = self._images_segmented[0:140]
            seg2 = self._images_segmented[420:1120]
            img = np.concatenate([img1,img2])
            seg = np.concatenate([seg1,seg2])
        
        elif pattern==6:
            img = self._images_original[280:1120]
            seg = self._images_segmented[280:1120]
        
        elif pattern==7:
            img = self._images_original[140:980]
            seg = self._images_segmented[140:980]
        
        if shuffle:
            idx = np.arange(img.shape[0])
            np.random.shuffle(idx)
            img, seg = img[idx], seg[idx]
        
        return DataSet(img, seg, self._image_palette,self._augmenter)
    
    def perm_val(self, pattern):
        if pattern == 0:
            return DataSet(self._images_original[840:980], self._images_segmented[840:980], self._image_palette,self._augmenter)
        elif pattern ==1:
            return DataSet(self._images_original[700:840], self._images_segmented[700:840], self._image_palette,self._augmenter)
        elif pattern==2:
            return DataSet(self._images_original[560:700], self._images_segmented[560:700], self._image_palette,self._augmenter)
        elif pattern==3:
            return DataSet(self._images_original[420:560], self._images_segmented[420:560], self._image_palette,self._augmenter)
        elif pattern==4:
            return DataSet(self._images_original[280:420], self._images_segmented[280:420], self._image_palette,self._augmenter)
        elif pattern==5:
            return DataSet(self._images_original[140:280], self._images_segmented[140:280], self._image_palette,self._augmenter)
        elif pattern==6:
            return DataSet(self._images_original[0:140], self._images_segmented[0:140], self._image_palette,self._augmenter)
        elif pattern==7:
            return DataSet(self._images_original[980:1120], self._images_segmented[980:1120], self._image_palette,self._augmenter)
    
    def perm_test(self, pattern):
        if pattern == 0:
            return DataSet(self._images_original[980:1120], self._images_segmented[980:1120], self._image_palette,self._augmenter)
        elif pattern ==1:
            return DataSet(self._images_original[840:980], self._images_segmented[840:980], self._image_palette,self._augmenter)
    ...
    def __call__(self, batch_size=20, shuffle=False, augment=True):
        """
